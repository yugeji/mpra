...
class MetaKerasModel(object):
    ...
    def build_model(self, dropout=0.01, L2=0.0001, L1=0.00001, pool_width=4, lr=0.00001, meta=True, weighted=True, merge_mode="sum", extra_conv = False, batch_norm=False):
        ...
        self.features_input = Input(shape=(self.DataObj.features_mat.shape[-1],),name="Epigenomic input")
    ...
    def train(self, X, X_RC, F, y, neg_weight, pos_weight, nb_epoch=60, batch_size=250, early_stopping=True, outside_eval = None, patience = 10, val_split=0.07, save=False, verbose=False):
        callbacks = []
        if early_stopping:
        ...
        if outside_eval is not None:
            history = self.model.fit(x=[X, X_RC, F], y=y, batch_size = batch_size, nb_epoch = nb_epoch, callbacks=callbacks,validation_data=outside_eval, shuffle=True, class_weight={0:neg_weight, 1:pos_weight}, verbose=verbose)
        else:
            history = self.model.fit(x=[X, X_RC, F], y=y, batch_size = batch_size, nb_epoch = nb_epoch, callbacks=callbacks,validation_split=val_split, shuffle=True, class_weight={0:neg_weight, 1:pos_weight}, verbose=verbose)
        #print history
        return history 

    def predict(self, X, X_RC, F, batch_size=200, verbose = 0):
...
class DoubleKerasModel(object):
    ...
    def train(self, X, X_RC, F, y, neg_weight, pos_weight, nb_epoch=60, batch_size=250, early_stopping=True, patience = 10, outside_eval = None, val_split=0.07, save=False, verbose=False):
        callbacks = []
        if early_stopping:
            self.EarlyStopping = keras.callbacks.EarlyStopping('val_loss', patience=patience)
            callbacks.append(self.EarlyStopping)
...
class SimpleKerasModel(object):
    ...
    def build_model(self, dropout=0.01, L1=0, L2=0.0001, pool_width=10, lr=0.00001, merge_mode='sum', conv_filters=150, extra_conv=False, weighted=True):
        ...
        self.features_input = Input(shape=(self.DataObj.features_mat.shape[-1],), name="Epigenomics Input")
        self.X_reduced = Flatten()(self.X_merged_out)
        self.X_feat_concat = merge([self.X_reduced, self.features_input], mode = 'concat', concat_axis = 1)
        self.X_feat_concat_dropout = Dropout(dropout)(self.X_feat_concat)
