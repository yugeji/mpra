...
class FilterZeroPad(object):
    def __init__(self, x, input_shape, motif_tensor, motif_shape, max_pool_size = 4, activation = T.nnet.relu, max_pool=True):
        #self.F = T.ftensor4('F')
        #self.X = T.ftensor4('X')
        ...
        self.W = theano.shared(value = np.ones(motif_shape[0], dtype = theano.config.floatX), name = 'W', borrow = True)
        self.b = theano.shared(value = np.zeros((motif_shape[0]), dtype = theano.config.floatX), name = 'b', borrow = True)
        ...
        self.regularize_params = [self.b_L1_norm, self.b_L2_norm, self.W_L1_norm, self.W_L2_norm]
        #self.model(pooling_size = (1,max_pool_size))

    #def model(self, activation = T.nnet.relu, max_pool = True, pooling_size = (1,4)):
        self.conv_out = conv2d(input= self.X, filters = self.F,  border_mode = 'valid', filter_flip=False)
        ...
        if max_pool == True:
            ...
            self.post_pool.dimshuffle(1,0,2,3)
            self.weighted_post_pool, _ = theano.scan(fn=ScaleFilter, sequences= [self.post_pool, self.W])
            self.weighted_post_pool.dimshuffle(1,0,2,3)
            output = activation(self.weighted_post_pool + self.b.dimshuffle('x', 0, 'x', 'x'))
        else:
            self.conv_out.dimshuffle(1,0,2,3)
            self.weighted_conv_out, _ = theano.scan(fn=ScaleFilter, sequences=[self.post_pool, self.W])
...
class FullyConnected(object):
    def __init__(self, input, n_in, n_out, minibatch_size, W=None, b=None, activation = T.nnet.relu, dropout = False):
        self.X = input
        if W is None:
            W_values = np.asarray(np.random.rand(n_in, n_out), dtype=theano.config.floatX)
...
class PoolingLayer(object):
    def __init__(self, input, filter_shape, input_shape, poolsize=(1, 3)):
        ...
        conv_out = conv.conv2d(input=input, filters=self.W, filter_shape=filter_shape, input_shape=input_shape)
...
class ConvModel(object):
    ...
    def fit(self,  dna_tensor, labels, test_dna_tensor, test_labels, motif_tensor,lambd=0.001, batch_size = 300, epochs = 20,custom_layer_max_pool_size = 4, FC_output = 400, gd_algorithm = "gd", gd_learning_rate = 0.00001):
        ...
        dna_tensor = theano.shared(dna_tensor, borrow = True)
        labels = theano.shared(labels, borrow = True)
        test_dna_tensor = theano.shared(test_dna_tensor, borrow =True)
        test_labels = theano.shared(test_labels, borrow = True)
        ...
        num_inputs_to_FC = num_first_layer_features * self.motif_tensor_shape[0]
        print "Number of inputs to FC layer is %i"%(num_inputs_to_FC)
        self.layers.append(FilterZeroPad(x=self.x,
                                         input_shape=(batch_size, self.dna_tensor_shape[1], self.dna_tensor_shape[2], self.dna_tensor_shape[3]),
        ...
        self.layers.append(FullyConnected(input = self.layers[-1].output, n_in = FC_output, n_out = 30, minibatch_size= self.batch_size))
        self.layers.append(LogisticRegression(input = self.layers[-1].output,n_in = 30, n_out = 1))
        ...
        if gd_algorithm == "adam":
            self.updates = adam(self.cost, self.full_params)
        elif gd_algorithm == "RMSprop":
            self.updates = RMSprop(self.cost, self.full_params)
        ...
        self.train_givens = {self.x: dna_tensor[self.index*self.batch_size:(self.index + 1)*self.batch_size],
                    self.y: labels[self.index*self.batch_size:(self.index + 1)*self.batch_size]
        }
        self.test_givens = {self.x: test_dna_tensor[self.index*self.batch_size:(self.index + 1)*self.batch_size],
                    self.y: test_labels[self.index*self.batch_size:(self.index + 1)*self.batch_size]
        }
        self.debug_givens = {self.x: dna_tensor[self.index * self.batch_size:(self.index + 1)*self.batch_size],
                             self.y: labels[self.index * self.batch_size:(self.index + 1)*self.batch_size]}
        self.train_model = theano.function([self.index], self.cost, updates = self.updates, givens = self.train_givens, on_unused_input = 'warn')
        self.debug_model = theano.function([self.index], self.layers[-1].binary_crossentropy(self.y), givens = self.debug_givens, on_unused_input = 'warn')
        self.test_model = theano.function([self.index], self.layers[-1].p_y_given_x, givens = self.test_givens, on_unused_input = 'warn')
        self.score_model = theano.function([self.index], self.cost, givens = self.test_givens, on_unused_input = 'warn')
        ...
        while self.cur_epoch < self.epochs:
            ...
            for minibatch_index in xrange(self.n_train_batches):
                ...
                if minibatch_index % 10 == 0:
                    print("Minibatch %i on epoch %i complete in %0.04f seconds with %0.04f training cost"%(minibatch_index, self.cur_epoch, time.time()-self.start_time), self.train_cost)
    ...
    def predict(self, dna_tensor, batch_size = 300):
        predict_start_time = time.time()
        self.predict_givens = {self.x: dna_tensor[self.index * batch_size: (self.index + 1 * batch_size)], self.f: self.motif_tensor}
        self.pred_model = theano.function(inputs=[self.index], outputs = self.layers[-1].y_pred,
